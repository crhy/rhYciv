...
def attribution(relative: str) -> tuple[str, str, str, str, str]:
    ...
    if relative.startswith("RaylibUI/FOSSart/"):
        ...
        elif "/Terrain/" in f"/{relative}" and any(
                f"/Terrain/{name}_{n}.png" in f"/{relative}"
                for name in ("desert", "plains", "grassland", "tundra", "glacier", "swamp", "jungle")
                for n in (1, 2, 3)):
            # Mirrored and patchworked from each terrain's own painting.
            generator = "scripts/prepare_terrain_variants.py"
